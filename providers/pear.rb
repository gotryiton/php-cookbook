...
def install_package(name, version)
  pear_shell_out("printf \"\\n\" | #{@bin} -d preferred_state=#{can_haz(@new_resource, "preferred_state")} install -a#{expand_options(@new_resource.options)} #{prefix_channel(can_haz(@new_resource, "channel"))}#{name}-#{version}")
  manage_pecl_ini(name, :create, can_haz(@new_resource, "directives")) if pecl?
end

def upgrade_package(name, version)
  pear_shell_out("printf \"\\n\" | #{@bin} -d preferred_state=#{can_haz(@new_resource, "preferred_state")} upgrade -a#{expand_options(@new_resource.options)} #{prefix_channel(can_haz(@new_resource, "channel"))}#{name}-#{version}")
  manage_pecl_ini(name, :create, can_haz(@new_resource, "directives")) if pecl?
end
...
def pecl?
  @pecl ||= begin
    return true if @new_resource.repo == :pecl
